...
`,
	...
	MsgVercelCustomEnvNotFound:      "Custom Environment %q がプロジェクト %s に存在しません（利用可能な Custom Environment: %s）",
	MsgVercelCustomEnvFetchFail:     "Custom Environment 一覧の取得に失敗",
	MsgVercelCustomEnvParseFail:     "Custom Environment 一覧のパースに失敗",
	MsgVercelCustomEnvNoneAvailable: "（なし — Vercel ダッシュボードで事前に作成してください）",
